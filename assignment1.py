...
def linear_regression(x, t, basis, reg_lambda=0, degree=0, mu=0, s=1):
    ...
    #design matrix = x
    phi =  ( inv((np.transpose(x)@ x ))) @ np.transpose(x) 
    ...
    sumerror = (t - np.transpose(w) @ np.transpose(x)).mean()
    train_err =  np.sqrt(sumerror) 

    return (w, train_err)



def design_matrix(basis=None, a =1):
...
def evaluate_regression(w, test_values, test_targets):
    ...
    t_est = np.transpose(w) @ np.transpose(test_values)
    sq_t_est = np.square((test_targets - np.transpose(w) @ np.transpose(test_values)))
    mean = sq_t_est/len(sq_t_est)
    root_t_est = np.sqrt(sq_t_est)
    #train_err =   
    err = train_err
